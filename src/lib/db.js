...
function rowToLead(row) {
  return {
    ...
    contractYears: row.contract_years || 0,
    totalContractValue: row.total_contract_value || 0,
...
function leadToRow(lead, userId) {
  return {
    ...
    contract_years: lead.contractYears,
    total_contract_value: lead.totalContractValue,
    annual_value: lead.annualValue,
    tags: lead.tags,
    value: lead.value,
    stage: lead.stage,
...
export async function updateTaskProduct(id, product) {
  const { error } = await supabase.from("tasks").update({ product }).eq("id", id);
  if (error) throw error;
}
